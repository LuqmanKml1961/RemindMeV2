...
- **Import actually shows "Imported!"** (`app/import/page.tsx`). After a successful import the page drops the URL fragment so a refresh can't import twice, but the success screen was still reading the fragment, so every import ended on "This link is invalid or corrupted" even though the data had been saved. The result is now kept in state.
